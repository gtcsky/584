...
//	LOG("\n batt start\n");
	if(getAdcStts()){
		...
		if(vAdcErrorTimes++<=10){
			LOG(" \n ad busy b%d  \n",vAdcErrorTimes);
			if(!osal_get_timeoutEx(command_center_TaskID, CCS_TEMP_CHECK_EVT)&&!osal_get_timeoutEx(command_center_TaskID, CCS_TEMP_VALUE_EVT)){
...
static void batt_adc_evt(adc_Evt_t* pev) {
	...
	if (pev->type == HAL_ADC_EVT_DATA) {
		...
		if (battVolt < 0) {
			...
			//displayFloat(64,3,battVolt,3,'V');
#if (BATT_VOLT_INFO_PRINT==1)
			LOG("\n battVolt=%d.%d",(uint16)battVolt,((uint16)(battVolt*1000))%1000);
#endif
			p_ctx->vbat_value = battVolt * (270 + 33) / 33; //4.7;//11.0;//4.3/0.9;
			p_ctx->vbat_value/=2;
//			if(getSystemStts())
				//p_ctx->vbat_value+=displayParams.adcGap;				//if system on
			p_ctx->vbat_value+=DEFAULT_ADC_GAP;
			batt_cal(*p_ctx);
