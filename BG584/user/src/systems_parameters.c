...
uint8 macAscii[12] = { 0 };
static const uint8 defaultAdvertData[] = { 0x02,   // length of this data
		GAP_ADTYPE_FLAGS,
		DEFAULT_DISCOVERABLE_MODE | GAP_ADTYPE_FLAGS_BREDR_NOT_SUPPORTED,

		// service
		0x03,
		GAP_ADTYPE_16BIT_MORE, LO_UINT16(DEVINFO_SERV_UUID), HI_UINT16(DEVINFO_SERV_UUID),

		// manufacturer specific
		0x09,// length of this data
		GAP_ADTYPE_MANUFACTURER_SPECIFIC, 'B', 'H', 0x01, 0x02, 0x03, 0x04, 0x05, 0x06 };
...
//connect inteval
		0x05,
		GAP_ADTYPE_SLAVE_CONN_INTERVAL_RANGE, LO_UINT16(DEFAULT_DESIRED_CONN_INTERVAL_100ms-16), HI_UINT16(DEFAULT_DESIRED_CONN_INTERVAL_100ms-16), LO_UINT16(
				DEFAULT_DESIRED_CONN_INTERVAL_100ms), HI_UINT16(DEFAULT_DESIRED_CONN_INTERVAL_100ms),
...
static const uint8 defaultModelNumber[] = { "BG584" };
...
static const uint8 defaultSoftwareVersion[] = { "584T2.10" };
...
static const uint8 defaultUserManualInfo[] = { "BG584A001B001C001" };
uint8 UserManualInfo[SYSPARAM_MAX_USER_MANUAL_SIZE + 2] = { 0 };
static const uint8 defaultOtaFile[] = { "BG584D001E001F001" };
...
static const uint8 defaultPNPID[] = { 1,                                      // Vendor ID source (1=Bluetooth SIG)
...
//		}
	...
	//SYSINFO_LOCALNAME, maybe it is  changed by user, write new data when  empty or data out of range.
	// LocalName[0] =0XFF;
	...
	//SYSINFO_DEVICENAME, maybe it is  changed by user, write new data when  empty or data out of range.
	...
	//AdvertData = default advert data+user broadcast data.
	...
	//ModelNumber, maybe it is  changed by user, write new data when  empty or data out of range.
	...
	//SerialNumber, maybe it is  changed by user, write new data when  empty or data out of range.
	...
	if (UserManualInfo[0] > SYSPARAM_MAX_USER_MANUAL_SIZE || UserManualInfo[0] == 0xff || UserManualInfo[0] == 0 || (sizeof(defaultUserManualInfo) - 1 != UserManualInfo[0])
			|| memcmp(&defaultUserManualInfo, &UserManualInfo[1], UserManualInfo[0])) {
	...
	//SYSINFO_INTERVALTIME, maybe it is  changed by user, write new data when  empty or data out of range.
...
//			LOG("update local Name");
		...
		break;
//	case SYSTEMS_PARAM_LOCALNAME:
//		if (value[0] <= SYSPARAM_MAX_LOCALNAME_SIZE) {		//valid data
//			osal_memcpy(&LocalName[1], &value[2], value[0]);
//			LocalName[0] =  value[0];
//			LocalName[LocalName[0] + 1] = 0;
//			for (index = 0; index < LocalName[0]; index++) {
//				LocalName[LocalName[0] + 1] ^= LocalName[index];
...
//
//		break;
//	case SYSTEMS_PARAM_DEVICENAME:
//		if (value[0] <= SYSPARAM_MAX_DEVICENAME_SIZE) {		//valid data
//			osal_memcpy(&DeviceName[1], &value[2], value[0]);
//			DeviceName[0] =  value[0];
//			DeviceName[DeviceName[0] + 1] = 0;
//			for (index = 0; index < DeviceName[0]; index++) {
//				DeviceName[DeviceName[0] + 1] ^= DeviceName[index];
//			}
...
//		}
//		break;
//	case SYSTEMS_PARAM_SERIALNUMBER:
//		if (value[0] <= SYSPARAM_MAX_SERIALNUMBER_SIZE) {		//valid data
//			osal_memcpy(&SerialNumber[1], &value[2], value[0]);
//			SerialNumber[0] =  value[0];
//			SerialNumber[SerialNumber[0] + 1] = 0;
//			for (index = 0; index < SerialNumber[0]; index++) {
//				SerialNumber[SerialNumber[0] + 1] ^= SerialNumber[index];
//			}
