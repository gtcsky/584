...
void turnOffAllLightEffect(void) {
	...
	turnOffColorTempLamp();
	turnOffRGBLamp();
	memset(&adjustParams,0,sizeof(adjustParams));
}
/*********************************************************************
 *
 * 	闪烁模式
...
//	vLEDStepCtrlTimeCnt = *(LedStruct.timeArray);
//}
u16 userAbsUint16(u16 da, u16 db) {
...
void colorAdjustGradual(colorStructType * colorStruct) {
	if (adjustParams.fIsPauseHuesInc) {
		if(++adjustParams.cnt>=ADJUST_MAX_CNT){
			...
			colorStruct->red+=adjustParams.deltaRed;
			colorStruct->green+=adjustParams.deltaGreen;
			colorStruct->blue+=adjustParams.deltaBlue;
		}
	} else {
//		if ((userAbsUint16(colorStruct->blue, adjustParams.backupColorStruct.blue) >= ADJUST_THRESHOLD) ||			\
//				(userAbsUint16(colorStruct->red, adjustParams.backupColorStruct.red) >= ADJUST_THRESHOLD)		\
//				|| (userAbsUint16(colorStruct->green, adjustParams.backupColorStruct.green) >= ADJUST_THRESHOLD)) {

			if((colorStruct->hues<=15)||(colorStruct->hues>=345)||((colorStruct->hues>=105)&&(colorStruct->hues<=135))||((colorStruct->hues>=225)&&(colorStruct->hues<=255))){
